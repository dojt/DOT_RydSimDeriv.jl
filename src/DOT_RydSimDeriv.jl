...
* `𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛺`   — smallest duration ``t^{\text{off}}-t^{\text{on}}`` which allows full range of 𝛺
* `𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛥`   — smallest duration ``t^{\text{off}}-t^{\text{on}}`` which allows full range of 𝛥
* `𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ`    — maximum of `𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛺` and `𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛥`
"""
...
function get_hw_data(hw ::HW_Descr{ℚ}) ::HW_Data
    ...
    𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛺 = δround_up( max( 𝛺_𝑢𝑝𝑡𝑖𝑚𝑒 ,𝛥𝑡ₘᵢₙ)  ; 𝛿=𝑡ᵣₑₛ)
    𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛥 = δround_up( max( 𝛥_𝑢𝑝𝑡𝑖𝑚𝑒 ,𝛥𝑡ₘᵢₙ)   ; 𝛿=𝑡ᵣₑₛ)
    ...
    return HW_Data(;
	           ...
	           𝑡ᵒᶠᶠₘₐₓ            = 𝑡ₘₐₓ - 𝑡ᵈᵒʷⁿ,
                   𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛺,
                   𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛥,
                   𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ        = max(𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛺,𝑡ᵒⁿ_𝑡ᵒᶠᶠₘᵢₙ𝛥),
	           𝑡ᵣₑₛ, 𝛥𝑡ₘᵢₙ, 𝑡ₘₐₓ
    )
end
...
function ␣get_𝛿𝑇(𝑋     ::Rad_per_μs_t{𝐐}
                 ;
                 𝛿𝑡ᵉᶠᶠ ::μs_t{𝐐},
                 𝑠ꜛ    ::Radperμs_per_μs_t{𝐐},
                 𝑠ꜜ    ::Radperμs_per_μs_t{𝐐}  ) ::μs_t{𝐐}    where{𝐐}
    ...
    let 𝛿𝑡¹ = 𝛿𝑡ᵉᶠᶠ − 𝑟⋅𝑋
        𝛿𝑡¹ ≥ 0 || throw(ArgumentError("𝛿𝑡ᵉᶠᶠ ($(𝛿𝑡ᵉᶠᶠ)) too small for pulse ($(𝑋))"))
    end
...
#    is_δrounded( 𝑡ᵒᶠᶠ ;𝛿=hw.𝑡ᵣₑₛ) || throw(ArgumentError("𝑡ᵒᶠᶠ must be multiple of HW 𝑡ᵣₑₛ"))
    is_δrounded( Δ_𝑡ᵒᶠᶠ;𝛿=hw.𝑡ᵣₑₛ) || throw(ArgumentError("𝑇 must be multiple of HW 𝑡ᵣₑₛ"))
