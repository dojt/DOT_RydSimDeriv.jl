...
* `𝑡ᵉᶠᶠₘᵢₙ𝛺`       — smallest possible value for 𝑡ᵉᶠᶠ to allow full range of 𝛺
* `𝑡ᵉᶠᶠₘᵢₙ𝛥`       — smallest possible value for 𝑡ᵉᶠᶠ to allow full range of 𝛥
* `𝑡ᵉᶠᶠₘₐₓ𝛺`       — largest possible value for 𝑡ᵉᶠᶠ to allow full range of 𝛺
* `𝑡ᵉᶠᶠₘₐₓ𝛥`       — largest possible value for 𝑡ᵉᶠᶠ to allow full range of 𝛥
"""
...
function get_hw_data(hw ::HW_Descr{ℚ}) ::HW_Data
    ...
    𝑡ᵉᶠᶠₘᵢₙ𝛺 =        δround_up( ( 1/𝛺_𝑚𝑎𝑥_𝑢𝑝𝑠𝑙𝑒𝑤 + 1/𝛺_𝑚𝑎𝑥_𝑑𝑜𝑤𝑛𝑠𝑙𝑒𝑤)/2 ⋅ 𝛺ₘₐₓ ; 𝛿=𝑡ᵣₑₛ)
    𝑡ᵉᶠᶠₘₐₓ𝛺 = 𝑡ₘₐₓ − δround_up( ( 1/𝛺_𝑚𝑎𝑥_𝑢𝑝𝑠𝑙𝑒𝑤 + 1/𝛺_𝑚𝑎𝑥_𝑑𝑜𝑤𝑛𝑠𝑙𝑒𝑤)/2 ⋅ 𝛺ₘₐₓ ; 𝛿=𝑡ᵣₑₛ)
    𝑡ᵉᶠᶠₘᵢₙ𝛥 =        δround_up( ( 1/𝛥_𝑚𝑎𝑥_𝑢𝑝𝑠𝑙𝑒𝑤 + 1/𝛥_𝑚𝑎𝑥_𝑑𝑜𝑤𝑛𝑠𝑙𝑒𝑤)/2 ⋅ 𝛥ₘₐₓ ; 𝛿=𝑡ᵣₑₛ)
    𝑡ᵉᶠᶠₘₐₓ𝛥 = 𝑡ₘₐₓ − δround_up( ( 1/𝛥_𝑚𝑎𝑥_𝑢𝑝𝑠𝑙𝑒𝑤 + 1/𝛥_𝑚𝑎𝑥_𝑑𝑜𝑤𝑛𝑠𝑙𝑒𝑤)/2 ⋅ 𝛥ₘₐₓ ; 𝛿=𝑡ᵣₑₛ)
...
function ␣get_𝛿𝑡ᵒⁿ(_𝑥     ::Rad_per_μs_t{𝐐}
    ...
    𝛿𝑡¹ ≥ 0 || throw(ArgumentError("𝛿𝑡ᵉᶠᶠ ($𝛿𝑡ᵉᶠᶠ) too small for pulse ($_𝑥)"))
...
function Evolution_Ω( 𝑡ᵒⁿ    ::μs_t{ℚ},
                      ...
                      ;
                      𝛥      ::Rad_per_μs_t,
                      Δ_𝑡ᵒᶠᶠ ::μ_s_t{ℚ}      = (-1//0)μs,
                      ε      ::ℝ,
                      hw     ::HW_Descr,
                      𝑇      ::μs_t{ℚ}       = (-1//0)μs )   ::Evolution_Ω

	(; 𝛺ₘₐₓ,𝛺ᵣₑₛ, 𝛥ₘₐₓ,𝛥ᵣₑₛ, 𝑡ᵈᵒʷⁿ,𝑡ᵒᶠᶠₘₐₓ, 𝑡ᵉᶠᶠₘᵢₙ𝛺, 𝑡ᵉᶠᶠₘₐₓ𝛺, 𝑡ᵣₑₛ,𝑡ₘₐₓ) =
            get_hw_data(hw)
    ...
    𝑡ᵉᶠᶠₘᵢₙ𝛺 ≤ 𝑡ᵉᶠᶠ             || throw(ArgumentError("Need 𝑡ᵉᶠᶠₘᵢₙ𝛺 ≤ 𝑡ᵉᶠᶠ"))
               𝑡ᵉᶠᶠ ≤ 𝑡ᵉᶠᶠₘₐₓ𝛺  || throw(ArgumentError("Need 𝑡ᵉᶠᶠ ≤ 𝑡ᵉᶠᶠₘₐₓ𝛺"))
    ...
    let
        ...
        𝛿𝑡¹ ≥ 0 || throw(ArgumentError("𝛿𝑡ᵉᶠᶠ ($𝛿𝑡ᵉᶠᶠ) too small for maximum parameter value ($𝛺ₘₐₓ)"))
...
function Evolution_Δ( 𝑡ᵒⁿ    ::μs_t{ℚ},
                      ...
                      ;
                      𝛺      ::Rad_per_μs_t,
                      Ω_𝑡ᵒᶠᶠ ::μ_s_t{ℚ}      = (-1//0)μs,
                      ε      ::ℝ,
                      hw     ::HW_Descr,
                      𝑇      ::μs_t{ℚ}       = (-1//0)μs )   ::Evolution_Δ

    (; 𝛺ₘₐₓ,𝛺ᵣₑₛ, 𝛥ₘₐₓ,𝛥ᵣₑₛ, 𝑡ᵈᵒʷⁿ,𝑡ᵒᶠᶠₘₐₓ, 𝑡ᵉᶠᶠₘᵢₙ𝛥, 𝑡ᵉᶠᶠₘₐₓ𝛥, 𝑡ᵣₑₛ,𝑡ₘₐₓ) =
        get_hw_data(hw)
    ...
    𝑡ᵉᶠᶠₘᵢₙ𝛥 ≤ 𝑡ᵉᶠᶠ             || throw(ArgumentError("Need 𝑡ᵉᶠᶠₘᵢₙ𝛥 ≤ 𝑡ᵉᶠᶠ"))
               𝑡ᵉᶠᶠ ≤ 𝑡ᵉᶠᶠₘₐₓ𝛥  || throw(ArgumentError("Need 𝑡ᵉᶠᶠ ≤ 𝑡ᵉᶠᶠₘₐₓ𝛥"))
    ...
    let
        ...
        𝛿𝑡¹ ≥ 0 || throw(ArgumentError("𝛿𝑡ᵉᶠᶠ ($𝛿𝑡ᵉᶠᶠ) too small for maximum parameter value ($𝛥ₘₐₓ)"))
